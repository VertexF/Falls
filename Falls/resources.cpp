...
uint32_t selectMemoryType(const VkPhysicalDeviceMemoryProperties& memoryProperties, uint32_t memoryTypeBit, VkMemoryPropertyFlags flags)
{
    ...
    {
        if ((memoryTypeBit & (1 << i)) != 0 && (memoryProperties.memoryTypes[i].propertyFlags & flags) == flags)
        {
            return i;
        }
    }
...
VkImageView createImageView(VkDevice device, VkImage image, VkFormat format, uint32_t mipLevels, uint32_t levelCount)
{
    ...
    createInfo.subresourceRange.baseMipLevel = mipLevels;
