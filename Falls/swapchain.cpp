                               ...
                               uint32_t width, uint32_t height, VkSwapchainKHR oldSwapchain)
{
    ...
    createInfo.presentMode = VSYNC ? VK_PRESENT_MODE_FIFO_KHR : VK_PRESENT_MODE_IMMEDIATE_KHR; // TODO: use immediate if available.
...
bool resizeSwapchainIfNecessary(Swapchain& result, VkPhysicalDevice physicalDevice, VkDevice device, VkSurfaceKHR surface,
    uint32_t familyIndex, VkFormat format, VkRenderPass renderPass)
{
    ...
    uint32_t newWidth = surfaceCap.currentExtent.width;
    uint32_t newHeight = surfaceCap.currentExtent.height;
