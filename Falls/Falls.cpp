...
VkRenderPass createRenderPass(VkDevice device, VkFormat colourFormat, VkFormat depthFormat, bool late) 
{
...
// C: Camera-space sphere centre. r: sphere radius. nearZ: nead clipping plane position negative.
vec4 getAxisAlignedBoundingBoxFast(const vec3& C, float r, float nearZ, float P00, float P11) 
{
    if (C.z < r + nearZ) 
    {
        return vec4();
    ...
    vec2 cx(-C.x, -C.z);
    vec2 vx = vec2(sqrt(dot(cx, cx) - r * r), r) / glm::length(cx);
    vec2 minx = mat2(vx.x, vx.y, -vx.y, vx.x) * cx;
    vec2 maxx = mat2(vx.x, -vx.y, vx.y, vx.x) * cx;

    vec2 cy(-C.y, -C.z);
    vec2 vy = vec2(sqrt(dot(cy, cy) - r * r), r) / glm::length(cy);
...
int main(int argc, const char** argv)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        char title[256];
        sprintf(title, "CPU : %.2f ms; GPU %.2f ms (cull: %.2f ms, pyramid: %.2f ms, cull late: %.2f); Triangles %.1fM; %.1fB tri/sec, %.1fB draws/sec, mesh shadering %s, frustum culling %s, occlusion culling %s, level-of-detail %s", 
