...
layout(binding = 1) readonly buffer Draw
{
    MeshDraw draws[];
};
...
void main()
{
    ...
    #if DEBUG
        uint mhash = hash(mi);
        vec3 mColour = vec3(float(mhash & 255), float((mhash >> 8) & 255), float((mhash >> 16) & 255)) / 255.0;
    #endif
    ...
    for (uint i = ti; i < vertexCount; i += GROUP)
    {
        ...
        colour[i] = vec4(normal * 0.5 + vec3(0.5), 1.0);

        #if DEBUG
            colour[i] = vec4(mColour, 1.0);
        #endif
    }
